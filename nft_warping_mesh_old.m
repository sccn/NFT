...
function nft_warping_mesh(subject_name, session_name, elec_file, nl, of, plotting, femmesh)
...
if plotting
    eeglab_plotmesh(Elem(:,2:4), Coord(:,2:4)); hold on; axis on;
...
kdm = find((dm < 2*mdm)); % & (dm > mdm - sdm)); % kdm gives the index of the electrodes close to the scalp
...
[Ptm, ind, Cscalp_w,Cskull_w,CCSF_w,Cbrain_w,W,A,e,LMm2,back,Escalp,Eskull,ECSF,Ebrain] = warping_main_function(of,Cscalp, ...
    Escalp, Eskull, ECSF, Ebrain,Cskull, CCSF, Cbrain, LMm, Fm, Fd, pos, kdm);
...
if plotting
    eeglab_plotmesh(Elem(:,2:4), Coord(:,2:4)); hold; axis on;
...
ssave.ind = ind;
save([of f '.sensors'], '-STRUCT', 'ssave')

% save the mesh
[Coord, Elem] = utilbem_add_mesh(Cscalp_w, Escalp, Cskull_w, Eskull);
[Coord, Elem] = utilbem_add_mesh(Coord, Elem, CCSF_w, ECSF);
if nl==4
    [Coord, Elem] = utilbem_add_mesh(Coord, Elem, Cbrain_w, Ebrain);
end
    
save([of subject_name '.bec'], 'Coord', '-ascii');


Info(1,1) = nl;
if nl==3
    Info(1,2) = size(Escalp,1)+size(Eskull,1)+size(ECSF,1);
    Info(1,3) = size(Cscalp_w,1)+size(Cskull_w,1)+size(CCSF_w,1);
elseif nl==4
    Info(1,2) = size(Escalp,1)+size(Eskull,1)+size(ECSF,1)+size(Ebrain,1);
    Info(1,3) = size(Cscalp_w,1)+size(Cskull_w,1)+size(CCSF_w,1)+size(Cbrain_w,1);
    Info(5,1) = 4;
    Info(5,2) = size(Ebrain,1);
    Info(5,3:4) = [4 3];
end
...
Info(3,3:4) = [2 1];
Info(4,1) = 3;
Info(4,2) = size(ECSF,1);
Info(4,3:4) = [3 2];



fid = fopen([of subject_name '.bei'], 'w');
fprintf(fid, '%d %d %d %d\r\n', Info');
fclose(fid);

if size(Elem,2) == 4
    fid = fopen([of subject_name '.bee'],'w');
    fprintf(fid, '%d %d %d %d\r\n', Elem');
    fclose(fid);
...
save([of f '_warping.mat'], 'warping_param'); % save the index, to use in IP
