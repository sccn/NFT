...
GEODESIC_DLL_IMPORT int distance_and_source(int algorithm_id,		//quickly find what source this point belongs to and what is the distance to this source
											 double* destination,			
															  ...
															  int** sources)
{
	...
	*distances = output_buffer.get<double>();
	...
	output_buffer1.allocate<int>(mesh->vertices().size());		//allocate space for sources
	*sources = output_buffer1.get<int>();
...
GEODESIC_DLL_IMPORT int trace_back(int algorithm_id,
									double* destination,
									double** path)
{
	geodesic::SurfacePoint point;
...
GEODESIC_DLL_IMPORT void propagate(int algorithm_id,
...
GEODESIC_DLL_IMPORT int new_mesh(int num_points,
								  double* points,	
								  int num_triangles,
								  int* triangles, 
								  int* num_edges, 
...
GEODESIC_DLL_IMPORT int new_algorithm(int mesh_id,
						               int type,
									   int subdivision)
{
...
GEODESIC_DLL_IMPORT void delete_mesh(int id)		//delete mesh and all related algorithms
{
